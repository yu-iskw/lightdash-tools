...
```bash
cd docs && openspec status --json 2>&1 || echo "NOT_INITIALIZED"
```

**If not initialized:**

> OpenSpec isn't set up in this project yet. Run `cd docs && openspec init` first, then come back to `/opsx:onboard`.
...
```bash
cd docs && openspec new change "<derived-name>"
```
...
```bash
cd docs && openspec instructions proposal --change "<name>" --json
```
...
```bash
mkdir -p docs/openspec/changes/<name>/specs/<capability-name>
```
