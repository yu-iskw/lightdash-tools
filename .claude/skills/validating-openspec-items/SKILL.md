...
1.  **Check Help**: Run `cd docs && npx @fission-ai/openspec@latest validate --help`.

## 2. Common Workflows

### Workflow: Validate a Specific Change

1.  Run `cd docs && npx @fission-ai/openspec@latest validate <change-name>`.

### Workflow: Validate All Changes

1.  Run `cd docs && npx @fission-ai/openspec@latest validate --changes`.

### Workflow: Strict Validation

1.  Use `--strict` to enable more rigorous checks:
    `cd docs && npx @fission-ai/openspec@latest validate <item-name> --strict`
