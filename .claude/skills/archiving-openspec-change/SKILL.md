...
1.  **Check Help**: Run `cd docs && npx @fission-ai/openspec@latest archive --help`.
...
1.  Run `cd docs && npx @fission-ai/openspec@latest archive <change-name>`.
2.  Use `-y` to skip confirmation prompts if you are confident:
    `cd docs && npx @fission-ai/openspec@latest archive <change-name> -y`
...
1.  For infrastructure or documentation changes that don't affect main specs, use `--skip-specs`:
    `cd docs && npx @fission-ai/openspec@latest archive <change-name> --skip-specs`
