...
1.  **Check Help**: Run `cd docs && npx @fission-ai/openspec@latest instructions --help`.

## 2. Common Workflows

### Workflow: Get Instructions for a Specific Artifact

1.  Run `cd docs && npx @fission-ai/openspec@latest instructions <artifact-name> --change <change-name>`.
    Example artifacts: `proposal`, `specs`, `design`, `tasks`.

### Workflow: Get Implementation Instructions

1.  Run `cd docs && npx @fission-ai/openspec@latest instructions apply --change <change-name>`.
...
**Command**: `cd docs && npx @fission-ai/openspec@latest instructions specs --change my-feature`
