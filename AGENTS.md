...
- **MCP Streamable HTTP is stateless (ADR-0019):** `NodeStreamableHTTPServerTransport` is created with `sessionIdGenerator: undefined`. Each POST creates a fresh transport+server. `SessionStore`, `create-session-store.ts`, `session-store.ts`, `redis-*`, `in-memory-preview-store.ts`, `create-preview-store.ts`, and `store/config.ts` are deleted. `LIGHTDASH_TOOLS_MCP_STORE` and `LIGHTDASH_TOOLS_MCP_REDIS_URL` are OBSOLETE_ENV_VARS (fail closed). GET/DELETE on persona paths return 405. MCP SDK does NOT support JSON-RPC batch arrays in streamable HTTP — each POST must carry exactly one JSON-RPC message. Multi-step flows (initialize → tools/call) require separate HTTP requests. **Form elicitation caps:** `initialize` capabilities are remembered in a principal- and persona-scoped process-local cache (`governance/client-capabilities-cache.ts`, keyed by OAuth subject/tokenHash + persona path; anonymous shared-key/`none` skips cache) and seeded onto each fresh `McpServer` so content-governance does not false-`ELICITATION_REQUIRED`; multi-replica should send per-request `_meta[CLIENT_CAPABILITIES_META_KEY]` or sticky-route `/content-governance/v1/mcp`.
