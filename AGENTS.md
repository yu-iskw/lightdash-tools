...
- pnpm v11: `overrides` live in `pnpm-workspace.yaml`; keep lockfile committed; `packageManager` is `pnpm@11.5.3`. Use exact pins (or `~`) for security bumps — `>=x.y.z` can pull the next major. `pnpm audit --fix` can rewrite an importer specifier to the manifest range and break `--frozen-lockfile`; the lockfile specifier must stay the override (`pnpm validate:lockfile-overrides`).
