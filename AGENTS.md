...
- Agent surface: MCP profiles own mounts via `tools: ToolModule[]` imports (ADR-0022); irrecoverable tool ids stay on `IRRECOVERABLE_TOOL_DENYLIST` (ADR-0004). AI-agent tools colocate by client resource (`agents.ts` inventory+preferences, `threads.ts` read+write, `evaluations.ts`) — do not add `chat.ts`/`preferences.ts`.
