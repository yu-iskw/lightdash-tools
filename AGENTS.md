...
- content-reader: prefer `list_verified_content` for trusted seeds (may be empty). Opaque saved SQL **results** allowed (ADR-0027); never SQL Runner / SQL body. UUID chart lookup uses the exact `uuids` filter; reader `get_chart`/`run_chart` treat semantic GET 404 as opaque SQL, but the shared `get_chart` on content-developer returns `CONTENT_NOT_FOUND`. `get_dashboard` executable tiles include a `run` handle — SQL tiles have **no** `chartUuid` (do not alias `savedSqlUuid`); use `run_dashboard_tile` / copy `tile.run`, not `run_chart`. Space search must not return membership `access` lists. `filterOverrides` enable disabled filters; wrong values yield 0 rows with no warning. `useCache=false` → `INVALID_PARAMETER_OVERRIDE`. No `export_chart_image`. Pin + `LIGHTDASH_TOOLS_ALLOWED_PROJECT_UUIDS` must both include the project.
