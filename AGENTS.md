...
- Agent surface: MCP profiles own mounts via `tools: ToolModule[]` imports (ADR-0022); irrecoverable tool ids stay on `IRRECOVERABLE_TOOL_DENYLIST` (ADR-0004). AI-agent tools colocate by client resource (`agents.ts` inventory+preferences, `router.ts` AI Router, `threads.ts` read+write, `evaluations.ts`) — do not add `chat.ts`/`preferences.ts`. Conversation writes and AI Router `routeAiAgent` / `route_agent` pass `retry: { maxRetries: 0 }` as HttpClient's 4th argument (not an Axios extra). **ai-agent-chat:** named/hint → preferences → `route_agent` → single-agent or ask user (never instruction/`enableContentTools` heuristics); URL/UUID grounding on create (ADR-0031). `create_agent_thread` requires `prompt` (empty `{}` fails — `getThread` INNER JOINs prompts); first turn create→generate, follow-ups message→generate (ADR-0030).
...
- Production HTTP requires non-empty `LIGHTDASH_TOOLS_MCP_PROFILES` (startup error lists valid ids); OAuth `/oauth/authorize` shows consent HTML before Lightdash (`/oauth/consent`). Stdio still requires `--profile` and ignores the env; non-production HTTP unset still mounts all. Prompt context default `compact`; override with `LIGHTDASH_TOOLS_MCP_PROMPT_CONTEXT` / `--prompt-context` (`compatible`|`embedded`; ADR-0025).
