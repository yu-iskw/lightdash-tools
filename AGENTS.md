...
- **MCP personas + shared tool registry (ADR-0006 / ADR-0008):** Shared tools live under `packages/mcp/src/tools/` (`registry.ts` plus scope folders `organization/`, `project/`, `space/`, `semantic/`, `query/`, `composition/`, and `lib/` helpers); personas under `packages/mcp/src/personas/<id>/v1/` own `toolIds`, prompts, playbooks, and a fixed HTTP `path`. Shipped personas: `semantic-layer` (`/semantic-layer/v1/mcp`), `organization-audit` (`/organization-audit/v1/mcp`, server name `lightdash-mcp-org-audit`), `content-reader` (`/content-reader/v1/mcp`, server name `lightdash-mcp-content`), and `content-developer` (`/content-developer/v1/mcp`, server name `lightdash-mcp-cdev`). Stdio: `lightdash-mcp` / `semantic-layer` default to semantic-layer; `lightdash-mcp organization-audit` / `lightdash-mcp content-reader` / `lightdash-mcp content-developer` select those personas. No process safety-mode / allowlist / dry-run on MCP. No free-form `LIGHTDASH_TOOLS_MCP_PATH`. OAuth PRM and `WWW-Authenticate` are persona-path-aware: path-specific metadata is at `/.well-known/oauth-protected-resource{persona.path}`; root PRM uses the default persona. `normalizePublicUrl` strips any shipped persona suffix from `PUBLIC_URL`.
...
- **`markPreviewValidated`/`confirm_preview` must bind to the exact resource:** Both take `expected: { resourceKind, resourceKey }` and throw `PREVIEW_STALE` on mismatch against the ledger entry — without this, any owned preview could unlock validation for a different chart/dashboard/space than the one it was diffed against. `validate_chart`/`validate_dashboard` still call the upstream Validate API and additionally bind to the exact `chartUuid`/`dashboardUuid`; `confirm_preview` is the bind-only path (no upstream call) for create/duplicate/tile/space/content-move flows with no existing uuid to validate against. `compare_*_versions` must call `resolveProjectScope` then verify resource ownership (`getSavedChart`/`getDashboard`) before fetching history, or a caller can read cross-project version history by uuid alone. `move_content`/`preview_space_changes` (content-move branch) must hash `contentTypes`/`chartSources` into the preview's `proposed` payload (`buildMoveContentProposal`), or an agent could preview one move shape and apply a different one under the same `previewId`.
- **`developer-content.ts` split into three files to stay under ~1000 lines:** `developer-content-shared.ts` (constants + `developerContext` + `wrapDeveloperHandler`), `developer-content-preview.ts` (the three `preview_*` tools, re-exported from `developer-content.ts` so `registry.ts` imports are unaffected), and `developer-content.ts` itself (validate/confirm/compare/writes via `registerChartUpsertTool`/`registerTileMutationTool` factories). Imports from a new local sibling module still need a blank line before `'../shared.js'`-style parent imports (different `import-x/order` groups).
