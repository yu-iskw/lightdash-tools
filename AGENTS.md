...
- **Shared project allowlist (CLI + MCP):** `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` is a comma-separated UUID ceiling (unset/empty = unrestricted). MCP has no project default — clients set `X-Lightdash-Project` or pass tool `projectUuid`. Deprecated alias `LIGHTDASH_TOOLS_MCP_AVAILABLE_PROJECT_UUIDS` still works when canonical is unset (startup warn).
