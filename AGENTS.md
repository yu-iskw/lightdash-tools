...
- Stdio: `lightdash-mcp stdio --profile <id>` only — no default; `http` mounts all profiles unless `LIGHTDASH_TOOLS_MCP_PROFILES` is set (comma-separated ids; unset = all). Restricted HTTP mounts also skip loading disabled ToolModules (preload enabled before `$PORT` listen; OAuth broker before listen). `getProfile` requires `preloadProfiles` first. Prompt context default `compact`; override with `LIGHTDASH_TOOLS_MCP_PROMPT_CONTEXT` / `--prompt-context` (`compatible`|`embedded`; ADR-0025).
...
- `parallel_validation`: pass booleans for both `trivialChangeDeclaration.codeReview.isTrivial` and `trivialChangeDeclaration.codeql.isTrivial`; the `codeql.isTrivial` schema rejects string values.
