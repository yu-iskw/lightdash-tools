...
- Agent surface: MCP profiles own mounts via `tools: ToolModule[]` imports (ADR-0022); irrecoverable tool ids stay on `IRRECOVERABLE_TOOL_DENYLIST` (ADR-0004). AI-agent tools colocate by client resource (`agents.ts` inventory+preferences, `threads.ts` read+write, `evaluations.ts`) — do not add `chat.ts`/`preferences.ts`. Conversation writes pass `retry: { maxRetries: 0 }` as HttpClient's 4th argument (not an Axios extra). **ai-agent-chat:** `create_agent_thread` requires `prompt` (empty `{}` fails upstream — `getThread` INNER JOINs prompts); first turn create→generate, follow-ups message→generate (ADR-0030 amends ADR-0029).
