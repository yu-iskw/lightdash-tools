...
- MCP tests are CJS under `tsc` — no `import.meta`; stdio smoke spawns `dist/bin.js` with an explicit profile arg. Nested semantic fieldIds: STRUCT on baseTable (`.`→`__`); ARRAY UNNEST join tables need `list_dimensions` `baseTableOnly=false`; `toFieldId` uses `split('.').join('__')` (es2017, not `replaceAll`). `compile_query` / `run_metric_query` fill missing FilterGroup/FilterRule `id`s (`ensureFilterIds`); unknown filter shapes pass through. SELECT-alias `isError` guard matches `` ` `` / `"` / plain `AS` aliases and skips when zero aliases parse (CTE/first-`FROM` heuristic).
