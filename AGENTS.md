...
- Changie: `kindFormat: '### {{.Kind}}'`; fragments use kind keys (`feat`/`fix`), not Keep-a-Changelog labels. After `changie merge`, Trunk/Prettier reformats CHANGELOG.md (`*` → `-`, blank lines). Do not run multiple `changie new` in the same second — identical `kind-YYYYMMDD-HHMMSS.yaml` names overwrite each other.
- Publish: GitHub Release tag must match workspace `package.json` versions before `pnpm -r publish`; otherwise publish exits 0 with `There are no new packages that should be published`. Workflow uses `--report-summary` and fails on empty `publishedPackages`. npm packument may lag a few minutes after a green publish.
