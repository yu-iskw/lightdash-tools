...
- **Semantic-layer MCP guidance:** `@lightdash-tools/semantic-layer-mcp` exposes prompts (`lightdash_semantic_explore`, `lightdash_semantic_compose_compile`, `lightdash_semantic_compile_debug`), a static playbook at `lightdash://playbooks/semantic-layer`, and unprefixed tools (`list_projects`, `get_project`, `list_explores`, `get_explore`, `list_dimensions`, `get_field_lineage`, `list_metrics`, `get_metric`, `compile_query`). Prompts are thin (role/context/deliverable + embedded playbook); procedure lives in the playbook only. `list_explores` returns `{ name, label, tags, databaseName?, schemaName? }` with optional `search`/`limit`; `list_dimensions` returns compact `{ name, label, table, type, fieldId }` where `fieldId` is `{table}_{name}` for `compile_query`. Warehouse table names are search hints, not explore IDs. Compile/discovery only — no run-query, SQL runner, content, or AI-agent tools. Tool surface is package-is-allowlist via thin `registerTool` (no `registerToolSafe` / safety-mode / dry-run / `ldt__` prefix). Stdio uses PAT (`LIGHTDASH_URL` + `LIGHTDASH_API_KEY`); HTTP uses experimental `lightdash-oauth` (ADR-0040) — OAuth client id/secret stay on the MCP **host**, not the server. Optional project pin is HTTP-only via `X-Lightdash-Project` → `governance.pinnedProjectUuid` (invalid UUID ignored; dynamic hide of list/set project tools not implemented yet). No `LIGHTDASH_TOOLS_PINNED_PROJECT` / `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` / `--pin-project` / `--projects` on this package. SDK v2 + `server.connect(StdioServerTransport)` / `NodeStreamableHTTPServerTransport` (ADR-0041); do not switch to `serveStdio` / `createMcpHandler` without an explicit protocol-era decision.
- **Semantic-layer MCP Docker (stdio):** Build from repo root: `docker build -f packages/semantic-layer-mcp/Dockerfile -t lightdash-semantic-layer-mcp:local .`. Root `.dockerignore` is a symlink to `packages/semantic-layer-mcp/.dockerignore` (Docker only reads ignore files from the context root). Image uses `pnpm deploy --prod --legacy` because this monorepo does not set `inject-workspace-packages`. Cursor: `command: docker`, `envFile: ${workspaceFolder}/.env`, forward with `-e LIGHTDASH_URL` / `-e LIGHTDASH_API_KEY` (no values). Never bake `.env` into the image; HTTP/`serve-http` is out of scope for this image.
- **Semantic-layer MCP Compose (HTTP + PAT):** `docker compose -f docker-compose.dev.yml --profile semantic-layer up --build` runs `serve-http` with `LIGHTDASH_TOOLS_MCP_AUTH_MODE=none` and `.env` PAT. Cursor points at `http://localhost:8080/mcp` (no OAuth `auth` block). `none` is rejected in production and requires `LIGHTDASH_API_KEY`.
