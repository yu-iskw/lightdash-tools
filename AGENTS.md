...
- **MCP personas + shared tool registry (ADR-0006 / ADR-0008):** Shared tools live under `packages/mcp/src/tools/` (`registry.ts` plus scope folders `organization/`, `project/`, `space/`, `semantic/`, and `lib/` helpers including org-audit registration asserts); personas under `packages/mcp/src/personas/<id>/v1/` own `toolIds`, prompts, playbooks, and a fixed HTTP `path`. Shipped personas: `semantic-layer` (`/semantic-layer/v1/mcp`) and `organization-audit` (`/organization-audit/v1/mcp`, server name `lightdash-mcp-org-audit`). Stdio: `lightdash-mcp` / `semantic-layer` default to semantic-layer; `lightdash-mcp organization-audit` selects the audit persona. No process safety-mode / allowlist / dry-run on MCP. No free-form `LIGHTDASH_TOOLS_MCP_PATH`. OAuth PRM and `WWW-Authenticate` are persona-path-aware: path-specific metadata is at `/.well-known/oauth-protected-resource{persona.path}`; root PRM uses the default persona. `normalizePublicUrl` strips any shipped persona suffix from `PUBLIC_URL`.
