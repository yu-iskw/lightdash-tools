...
- **Env vars use LIGHTDASH_TOOLS\_ prefix:** Project-specific vars use this prefix to distinguish from official Lightdash vars (`LIGHTDASH_URL`, `LIGHTDASH_API_KEY`). CLI uses `LIGHTDASH_TOOLS_SAFETY_MODE`, `LIGHTDASH_TOOLS_DRY_RUN`, and shared `LIGHTDASH_TOOLS_ALLOWED_PROJECTS`; MCP uses persona + pin + the same optional `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` ceiling + optional `LIGHTDASH_TOOLS_AUDIT_LOG` (no process safety-mode / dry-run). See [ADR-0009](docs/adr/0009-cross-cutting-conventions.md).
- **Shared project allowlist (CLI + MCP):** `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` is a comma-separated UUID ceiling (unset/empty = unrestricted). MCP has no project default — clients set `X-Lightdash-Project` or pass tool `projectUuid`. Removed `LIGHTDASH_TOOLS_MCP_AVAILABLE_PROJECT_UUIDS` fails startup if still set.
...
- **Slim ADRs:** Binding decisions live under [`docs/adr`](docs/adr) (currently 0001–0017); see the old→new mapping in [`docs/adr/README.md`](docs/adr/README.md).
- **Content-developer preview ledger (ADR-0014 / ADR-0016):** `packages/mcp/src/policy/preview-ledger.ts` gates every SAFE_WRITE behind a validated, unexpired, session-owned `previewId` whose `contentHash` (sha256 of stable-stringified `{proposed,baseline}`) matches the apply payload. Apply path: `claimPreviewForApply` (`validated` → `applying` via CAS) → mutate → `markPreviewApplied` (delete) or `releaseOrReconcilePreview` (known 4xx → `validated` for retry; uncertain → `reconciliation_required`). Never delete-before-I/O as the sole path. Store is pluggable: `LIGHTDASH_TOOLS_MCP_STORE=memory` (default, process-local) vs `redis` for multi-instance HTTP (`packages/mcp/src/store/`). Lookup helpers (`getOwnedPreview`) must NOT prune expired entries before reading — pruning first turns an expected `PREVIEW_EXPIRED` into a false `PREVIEW_REQUIRED`. Redis preview keys use logical `expiresAt` **plus** a short grace TTL (`PREVIEW_REDIS_EXPIRY_GRACE_MS`) so keys remain readable after logical expiry. Resolve move manifests with content `uuids` (exact), not `search: uuid`. `PreviewLedgerError` reports its reason via a `code` property — tests must assert `err.code`, not `toThrow(/CODE/)`.
- **Redis OAuth DCR cap:** `RedisOAuthBrokerStore.registerClient` must enforce `maxClients` (ZSET index + Lua), matching in-memory — do not rely on client-key TTLs alone.
