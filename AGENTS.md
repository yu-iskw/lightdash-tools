...
- pnpm v11: `overrides` live in `pnpm-workspace.yaml`; keep lockfile committed; `packageManager` is `pnpm@11.5.3`. Use exact pins (or `~`) for security bumps — `>=x.y.z` can pull the next major. `pnpm audit --fix` can rewrite an importer specifier and break `--frozen-lockfile`; that install is the gate, not a custom lockfile parser.
...
- Agent surface: MCP profiles own mounts via `tools: ToolModule[]` imports (ADR-0022); irrecoverable tool ids stay on `IRRECOVERABLE_TOOL_DENYLIST` (ADR-0004). AI-agent tools colocate by client resource (`agents.ts` inventory+preferences+create/update, `documents.ts` knowledge CRUD, `router.ts` AI Router, `threads.ts` read+write, `evaluations.ts`) — do not add `chat.ts`/`preferences.ts`. **ai-agent-ops:** `create_project_agent` dual-gate — form elicitation when capable, else `preview_create_agent`→`confirm_create_agent`→create with `createConfirmToken` (ADR-0034); knowledge docs via `create_agent_document` (inline JSON, 20KB, agent-scoped route, ADR-0035); secure defaults shared with CLI/agentops; tags are dbt/Lightdash explore tags (not explore names) — verify with `get_explore_access_summary`; agent delete stays CLI. Conversation writes and AI Router pass `retry: { maxRetries: 0 }` as HttpClient's 4th argument. **ai-agent-chat:** named/hint → preferences → `route_agent` → single-agent or ask user; `create_agent_thread` requires `prompt`; first turn create→generate, follow-ups message→generate (ADR-0030).
...
- content-reader: prefer `list_verified_content` (GET `…/content-verification`) for trusted seeds; `search_content` has no `verifiedOnly` filter. Dashboard `sql_chart` tiles run via `run_dashboard_tile` (ADR-0028); standalone `run_chart` SQL stays off; `dateZoom` is ignored on SQL tiles. Authored SQL reveal is opt-in `includeArtifacts=["sql"]` as a separate MCP resource (ADR-0032); run/poll default `['data']` keeps rows out of the summary envelope.
...
- MCP tests are CJS under `tsc` — no `import.meta`; stdio smoke spawns `dist/bin.js` with an explicit profile arg. Nested semantic fieldIds: STRUCT on baseTable (`.`→`__`); ARRAY UNNEST join tables need `list_dimensions` `baseTableOnly=false` (ids like `{base}__payments_payment_method`, not `{base}_payments__…`); `toFieldId` uses `split('.').join('__')` (es2017, not `replaceAll`). `compile_query` / `run_metric_query` fill missing FilterGroup/FilterRule `id`s (`ensureFilterIds`) and default missing `tableCalculations` / `sorts` to `[]`. Filter operators are `notNull`/`isNull`, not `isNotNull`. `compile_query` sets `isError` on empty SELECT, compiled SQL `/* ERROR:` comments (unknown filter fieldIds), or missing SELECT aliases. SELECT-alias guard matches `` ` `` / `"` / plain `AS` aliases and skips when zero aliases parse (CTE/first-`FROM` heuristic).
