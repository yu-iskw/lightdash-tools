...
- **MCP bare-server tool listing (SDK detail):** In `@modelcontextprotocol/sdk`, the `tools/list` request handler is only registered after at least one tool is registered. For tests that assert “zero tools until registered”, prefer inspecting `server._registeredTools` (or register tools before using an MCP client).
- **MCP library vs tools exports:** `@lightdash-tools/mcp` exports server factories, personas, and stdio helpers only. Import individual `register*Tool` functions from `@lightdash-tools/mcp/tools` in custom persona packages so the main entry does not pull the full tool module graph.
...
- `pnpm-workspace.yaml` maintains supply-chain mitigations for global pnpm policies: `semver: '>=7.8.4'` override, expanded `minimumReleaseAgeExclude`, and `vite: 8.0.16` pin.
