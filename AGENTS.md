...
- **Serving persona is `bindServerPersona`:** `registerCapabilities` binds the persona on the MCP server before tools register. Audit, content envelopes (`contentReaderEnvelope` `context.persona`), and `get_project` capability branching read `getServerPersona` / `requireServerPersona` — do not thread a separate `{ personaId }` options bag through `registerToolsByIds`.
...
- **Data-analyst persona (ADR-0020):** Unsaved Explore-style analysis (9 tools). Discover explores/fields, optional `compile_query`, then `run_metric_query` via `POST …/query/metric-query` with the same bounded row/concurrency caps as content-reader. Not on content-reader (saved only) or semantic-layer (compile only). No SQL, underlying-data, downloads, or content mutation. Server name `lightdash-mcp-analyst`; path `/data-analyst/v1/mcp`. Inventory: [docs/personas/data-analyst/inventory.md](docs/personas/data-analyst/inventory.md).
