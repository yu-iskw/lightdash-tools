...
- Stateless MCP HTTP (ADR-0019): no SessionStore/Redis; obsolete store/redis env fail closed; OAuth broker is in-memory (one replica or dedicated `/oauth/*` service — GCLB affinity does not pin Cloud Run instances). HTTP volume/TLS/Host/WAF are operator edge (ADR-0032); do not delete Origin, dual-leg OAuth, or per-user query budgets to “delegate to Armor.” Empty `ALLOWED_ORIGINS` is the hosted default.
