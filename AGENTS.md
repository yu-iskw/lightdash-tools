...
- Stdio: `lightdash-mcp stdio --profile <id>` only — no default; `http` mounts all profiles unless `LIGHTDASH_TOOLS_MCP_PROFILES` is set (comma-separated ids; unset = all). Restricted HTTP mounts skip disabled ToolModules (`preloadProfiles` before `$PORT` listen; OAuth broker before listen; `getProfile` needs preload — ADR-0024). Prompt context default `compact`; override with `LIGHTDASH_TOOLS_MCP_PROMPT_CONTEXT` / `--prompt-context` (`compatible`|`embedded`; ADR-0025).
