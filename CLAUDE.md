...
- [2026-08-03]: Sessionless HTTP content-governance: bridge `initialize` form-elicitation caps via principal-scoped process-local cache (`client-capabilities-cache.ts`) and seed `_clientCapabilities` on each fresh `McpServer` so the 2025 SDK elicitation shim still works; prefer per-request `_meta[CLIENT_CAPABILITIES_META_KEY]` for multi-replica.
