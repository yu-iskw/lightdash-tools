...
- [2026-08-02]: Create-preview race: `consumeValidatedPreview` rejects when `entry.baseline == null` but apply `currentBaseline` has uuid/updatedAt (`PREVIEW_STALE`). `duplicate_chart` re-reads `newSlug` before consume. `duplicate_dashboard` omits `spaceUuid` (duplicateFrom params are name/desc only); relocate via `move_content`.
- [2026-08-02]: Content-developer unlock is `confirm_preview` only (create/update/duplicate/tiles/move). `validate_*` is a saved-UUID health check and does not call `markPreviewValidated`. Create chart previews with an occupied slug return `CHART_SLUG_EXISTS`; `duplicate_dashboard` re-reads source baseline at apply.
