...
- [2026-08-01]: Fixed a preview-gate binding gap: `markPreviewValidated` now takes `expected: { resourceKind, resourceKey }` and throws `PREVIEW_STALE` on mismatch (previously any owned preview could be marked validated regardless of resource). Added `confirm_preview` as the bind-only counterpart to `validate_chart`/`validate_dashboard` for create/duplicate/tile/space/content-move flows with no upstream validate API. Also added project-scope ownership checks to `compare_chart_versions`/`compare_dashboard_versions` and hashed `contentTypes`/`chartSources` into `move_content` preview payloads. Split `developer-content.ts` into `developer-content-shared.ts` + `developer-content-preview.ts` + `developer-content.ts` to stay well under 1000 lines, using factory helpers; kept all `register*` export names stable via a re-export barrel so `registry.ts` needed no changes.
- [2026-08-01]: Content-developer ADR-0014 handlers (`packages/mcp/src/tools/project/developer-content.ts`, `policy/preview-ledger.ts`, `policy/content-developer.ts`, `tools/project/developer-helpers.ts`): preview_space_changes is polymorphic — it also serves as the preview for `move_content` (pass `itemUuids` + `targetSpaceUuid` instead of `spaceUuid` + `changes`, emitting `resourceKind: 'content-move'`) since the catalog has no dedicated content-move preview endpoint. `resourceKey` convention for chart/dashboard previews is `slug ?? uuidOrSlug ?? 'new'`; tile-op tools (`add/move/remove/resize_dashboard_tile`) compute `nextTiles` locally with pure helpers then `consumeValidatedPreview` against `{ tiles: nextTiles }` — the agent must have previewed that exact payload first, or it's rejected as `PREVIEW_STALE`. Added `VALIDATE_SAFETY` (mutability 'none', resultCapability 'metadata') and `COMPARE_SAFETY` (mutability 'none', resultCapability 'diff') alongside `PREVIEW_SAFETY`/`WRITE_SAFETY` in `content-developer.ts` for accurate per-tool-family classification. Extended `reader-tool-helpers.ts` `BLOCKED_POLICY_CODES` with the five `PREVIEW_*` codes so preview-gate denials audit as `blocked`.
