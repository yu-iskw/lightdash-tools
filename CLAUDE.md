...
- [2026-08-17]: content-reader dashboard tiles: `get_dashboard` no longer aliases SQL `savedSqlUuid` into `chartUuid`. Executable tiles expose copy-paste `run` (`lightdash_run_dashboard_tile` + `tileUuid`). `run_chart`/`get_chart` 404s add ADR-0025 `recovery` + `explain-run` playbookUri.
