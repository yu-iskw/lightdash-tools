...
- [2026-08-26]: content-reader SQL bodies are `secret.sql_body` (ADR-0032): omit from summary; attach via `includeArtifacts=["sql"]` as MCP `resource` parts. Run/poll (including data-analyst `run_metric_query` + shared `get_query_result`) default `includeArtifacts=['data']` so rows leave the compact envelope — do not expect `data.rows` in the summary. Per-tool allowlists must not advertise `'sql'` on tools that cannot attach it; discover SQL charts without SqlChart GET unless opted in. Standalone `run_chart` SQL execution stays denied (`canExecuteSqlCharts=false`; `canRevealSqlBodies=true`).
