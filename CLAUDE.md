...
- [2026-07-31]: Organization-audit MCP persona ships at `/organization-audit/v1/mcp` with server name `lightdash-mcp-org-audit` (60-char limit). Stdio: `lightdash-mcp organization-audit`. No unused-content API — use user-activity + content views. Space access composed from space list/get. Prefer project schedulers list over per-chart/dashboard.
