...
- [2026-07-30]: `@lightdash-tools/semantic-layer-mcp` ships three user-controlled prompts plus static playbook `lightdash://playbooks/semantic-layer` (embedded in `prompts/get`). Keep `playbook.md` next to the compiled `playbook.js` (`tsc` + `cp` in package build). Package-is-allowlist (no safety-mode); stdio PAT + HTTP `lightdash-oauth` (host holds OAuth client secrets). Same ADR-0041 rule as monolith MCP: SDK v2 + `server.connect(StdioServerTransport)` / Streamable HTTP transport; do not default to `serveStdio` / `createMcpHandler`.
