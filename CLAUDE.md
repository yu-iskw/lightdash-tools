...
- [2026-08-03]: MCP and CLI share `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` as the optional project ceiling. MCP has no process default (`LIGHTDASH_TOOLS_PROJECT_UUID` removed); resolve pin → tool `projectUuid` → `PROJECT_SCOPE_REQUIRED`. Deprecated alias `LIGHTDASH_TOOLS_MCP_AVAILABLE_PROJECT_UUIDS` warns at startup. Still ignore SAFETY_MODE/DRY_RUN on MCP.
