...
- [2026-08-26]: content-reader SQL bodies are `secret.sql_body` (ADR-0032): omit from summary; attach via `includeArtifacts=["sql"]` as MCP `resource` parts. Run/poll default `includeArtifacts=['data']` so rows leave the compact envelope. Standalone `run_chart` SQL execution stays denied (`canExecuteSqlCharts=false`; `canRevealSqlBodies=true`).
