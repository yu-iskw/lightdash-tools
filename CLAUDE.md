...
- [2026-07-30]: `@lightdash-tools/semantic-layer-mcp` optional project pin: HTTP `X-Lightdash-Project` or `LIGHTDASH_TOOLS_PINNED_PROJECT` / `--pin-project`. Invalid UUIDs ignored. CLI pin uses static override (like `--projects`), not env mutation; subcommands must merge parent opts (`resolveCliOptions`) under Commander 15. Hide list/set project tools only once those tools are registered.
