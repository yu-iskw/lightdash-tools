...
- [2026-08-03]: Sessionless HTTP content-governance: bridge `initialize` form-elicitation caps via principal- and persona-scoped process-local cache (`client-capabilities-cache.ts`, skip anonymous shared-key/`none`) and seed `_clientCapabilities` on each fresh `McpServer` so the 2025 SDK elicitation shim still works; prefer per-request `_meta[CLIENT_CAPABILITIES_META_KEY]` for multi-replica.
- [2026-08-03]: ADR-0019 follow-up: destructive MRTR `requestState` binds auth `subject` (not process `sessionId`) so accept works across sessionless replicas; query get/cancel treat the in-memory ledger as optional budget bookkeeping (`findQueryLedgerEntry`) while `queryUuid` remains the durable handle; concurrency budgets prefer `userKey`/subject and skip the collapsed process session bucket when authenticated.
- [2026-08-03]: ADR-0019: MCP HTTP is sessionless (no Redis, no SessionStore). Content-developer uses HMAC `previewToken` (subject-bound, hash of proposed+baseline); confirm mints validated token; apply re-sends proposed. Promote drops confirmation-claim store (TTL + RESOURCE_CHANGED). OAuth broker stays in-memory — sticky `/oauth/*` or single replica. `LIGHTDASH_TOOLS_MCP_STORE`/`REDIS_URL` fail closed.
- [2026-08-03]: MCP Streamable HTTP is now stateless per ADR-0019. `NodeStreamableHTTPServerTransport` created with `sessionIdGenerator: undefined` — fresh server per POST, no SessionStore, no cleanup timer. STORE/REDIS_URL env vars are obsolete and fail closed. GET/DELETE on persona paths return 405. MCP SDK streamable HTTP does NOT support JSON-RPC batch arrays — each POST carries one JSON-RPC message. Multi-step flows (init → tools/call) need separate HTTP requests and cannot be batched; integration tests now focus on per-request auth checks only.
- [2026-08-03]: MCP and CLI share `LIGHTDASH_TOOLS_ALLOWED_PROJECT_UUIDS` as the optional project ceiling (renamed from `ALLOWED_PROJECTS`; legacy name fails closed). MCP has no process default (`LIGHTDASH_TOOLS_PROJECT_UUID` removed); resolve pin → tool `projectUuid` → `PROJECT_SCOPE_REQUIRED`. Removed `LIGHTDASH_TOOLS_MCP_AVAILABLE_PROJECT_UUIDS` also fails startup if set. Still ignore SAFETY_MODE/DRY_RUN on MCP.
- [2026-08-03]: `pnpm --filter <pkg> test:fast` is not reliable across this monorepo because some packages do not define `test:fast` (or any package-local test script). For file-scoped targeted tests, use root `pnpm exec vitest run <files...>` instead.
- [2026-08-03]: MCP and CLI share `LIGHTDASH_TOOLS_ALLOWED_PROJECT_UUIDS` as the optional project ceiling (renamed from `ALLOWED_PROJECTS`; legacy name fails closed). MCP has no process default (`LIGHTDASH_TOOLS_PROJECT_UUID` removed); resolve pin → tool `projectUuid` → `PROJECT_SCOPE_REQUIRED`. Removed `LIGHTDASH_TOOLS_MCP_AVAILABLE_PROJECT_UUIDS` also fails startup if set. Still ignore SAFETY_MODE/DRY_RUN on MCP.
...
- [2026-03-06]: All project-specific env vars use `LIGHTDASH_TOOLS_` prefix: `LIGHTDASH_TOOLS_ALLOWED_PROJECT_UUIDS`, `LIGHTDASH_TOOLS_DRY_RUN`, `LIGHTDASH_TOOLS_AUDIT_LOG`, `LIGHTDASH_TOOLS_SAFETY_MODE`. See ADR-0035.
...
- [2026-07-30]: `@lightdash-tools/semantic-layer-mcp` project pin is HTTP-only (`X-Lightdash-Project` → `governance.pinnedProjectUuid`). Removed unused `LIGHTDASH_TOOLS_PINNED_PROJECT` / `LIGHTDASH_TOOLS_ALLOWED_PROJECT_UUIDS` and `--pin-project` / `--projects` from this package (monolith `packages/mcp` keep allowlist).
