...
5. After apply: list chart UUIDs still dashboard-owned (`dashboardSlug`) but **not** tiled. Soft-delete those leftovers via **content-governance** — not this persona ([dashboard-owned charts stay off search/spaces](https://docs.lightdash.com/get-started/exploring-data/dashboards#create-a-new-chart)).
