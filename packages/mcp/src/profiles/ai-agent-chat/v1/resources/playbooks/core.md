...
Use an **existing** Lightdash AI Agent as the authenticated Lightdash user. Discover an accessible agent (preference or **AI Router**), **default to a new conversation**, and ask Lightdash to generate the managed-agent response. Continue only with a known own-thread UUID (or explicit own-history lookup when the user asks).
...
- Do not invent an agent pick by matching instruction text, `enableContentTools`, tags, or fuzzy name similarity — use named/`agentHint`, preferences, `route_agent`, single-agent fallback, or ask the user.

## Allowed tools (`lightdash_*`)

| Tool                          | Use for                                                           |
| ----------------------------- | ----------------------------------------------------------------- |
| `list_project_agents`         | Discover agents (named exact match; single-agent Router fallback) |
| `get_user_agent_preferences`  | Resolve the caller's per-project default agent                    |
| `route_agent`                 | AI Router best-fit when no name/default (then create → generate)  |
...
`list_project_agents` returns the upstream agent summary. It may include instruction, integrations, and flags such as `enableSqlMode`. Treat those as Lightdash configuration facts, not switches this profile can change — and **not** as a host-side ranking signal.

`route_agent` returns an org AI Router decision for **managed-agent conversation**. It does **not** activate Data MCP session agent context (`set_agent`). Upstream may persist a decision row (including the prompt). When `nextAction` is `show_picker`, ask the user. Selection-only but **not** annotated read-only.
...
`create_agent_thread` (with prompt), `create_agent_thread_message`, and `generate_agent_response` are non-idempotent. After an ambiguous network failure, do not blindly create again. New conversations: create with prompt → generate. Follow-ups: message → generate. If the user message already exists, retry `generate_agent_response` only. `route_agent` persists a decision upstream — prefer not to retry blindly after an ambiguous failure; if the host already has `decisionUuid` / `suggestedAgentUuid`, reuse that pick.
