...
export function registerProjectRoleAssignmentsCommand(program: Command): void {
  ...
  assignCmd
    ...
    .action(async (projectUuid: string, userUuid: string, cmd: Command) => {
  ...
  assignCmd
    ...
    .requiredOption('--role-id <roleId>', 'Role ID to assign')
    .action(async (projectUuid: string, groupId: string, cmd: Command) => {
      const opts = cmd.opts() as { roleId: string };
      try {
        const client = getClient();
  ...
  unassignCmd
    ...
    .action(async (projectUuid: string, userUuid: string) => {
      try {
        const client = getClient();
  ...
  unassignCmd
    ...
    .action(async (projectUuid: string, groupId: string) => {
      try {
        const client = getClient();
