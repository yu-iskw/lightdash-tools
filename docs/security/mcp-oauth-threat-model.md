...
| Lightdash data and tool results | Must be scoped by Lightdash permissions, persona tool surface, optional HTTP project pin, and optional `LIGHTDASH_TOOLS_ALLOWED_PROJECTS`. |
...
| Shared PAT overreach                       | High   | PAT/shared-key is secondary; prefer OAuth. Persona tool surface + Lightdash RBAC (+ optional `X-Lightdash-Project` pin and/or `LIGHTDASH_TOOLS_ALLOWED_PROJECTS`).                                                                                          |
...
- [ ] Pin project via client/gateway `X-Lightdash-Project` when operators need a per-request project, and/or set `LIGHTDASH_TOOLS_ALLOWED_PROJECTS` for a process-level hard allowlist ([ADR-0008](../adr/0008-mcp-request-scope-and-hardening.md)).
...
- [ ] Do not rely on `LIGHTDASH_TOOLS_SAFETY_MODE` / `DRY_RUN` for MCP — those are CLI-only (`LIGHTDASH_TOOLS_ALLOWED_PROJECTS` is shared with MCP as the project ceiling).
