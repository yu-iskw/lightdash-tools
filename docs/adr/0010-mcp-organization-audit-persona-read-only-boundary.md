...
1. Ship a separate **`organization-audit`** persona at fixed path `/organization-audit/v1/mcp` with an explicit tool allowlist of **read-only primitive** inventory/access/content/scheduler tools only. Multi-step audits are orchestrated by the host via prompts and the playbook resource — not via composed `audit_*` MCP tools.
...
- Agents (or humans) paginate and join primitives; coverage honesty depends on `pagination.complete` and playbook caps, not a single mega-tool.
