...
6. **SDK:** Pin exact `@modelcontextprotocol/server@2.0.0` and `@modelcontextprotocol/node@2.0.0` (plus peers such as `hono`). Stdio: `server.connect(StdioServerTransport)`. HTTP: stateful `NodeStreamableHTTPServerTransport`. Do **not** switch to `serveStdio` / `createMcpHandler` without a follow-up decision.
