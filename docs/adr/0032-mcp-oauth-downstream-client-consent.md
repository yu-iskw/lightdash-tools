...
That static Lightdash client plus Dynamic Client Registration (DCR) is a **confused-deputy** class. An MCP client can register itself, send the user to `/oauth/authorize`, and ride the already-trusted Lightdash application without the user ever seeing _which_ MCP client they are about to authorize. MCP Security Best Practices require **per-client consent** before forwarding to the third-party authorization server. [ADR-0026](0026-mcp-oauth-dual-leg-token-boundary.md) closed token passthrough and audience binding; it explicitly left this control unimplemented.
