...
Supersedes former ADR-0016 (pluggable Redis/memory ephemeral store) — Redis is **not** current law.

Amends [7. MCP HTTP transport, OAuth broker, SDK v2](0007-mcp-http-transport-auth-modes-sdk-v2.md), [12. MCP content-reader profile](0012-mcp-content-reader-profile-saved-content-execution-boundary.md), [14. MCP content-developer profile](0014-mcp-content-developer-profile-mutation-boundary.md), [15. MCP content-governance soft-delete](0015-mcp-content-governance-profile-elicitation-required-soft-delete-boundary.md), [17. MCP content-governance promote](0017-mcp-content-governance-dashboard-promote-elicitation-boundary.md)
...
A prior design (`LIGHTDASH_TOOLS_MCP_STORE=memory|redis`) shared preview ledger, OAuth pending, and a session index across instances. That recreated sticky/shared-store operational cost the protocol revision removes. Handles must not be treated as authentication; bind them to the authenticated principal ([state handle hijacking](https://modelcontextprotocol.io/docs/2026-07-28/tutorials/security/security_best_practices#state-handle-hijacking)).
...
1. **No Redis / no pluggable ephemeral store.** `LIGHTDASH_TOOLS_MCP_STORE`, `LIGHTDASH_TOOLS_MCP_REDIS_URL`, and Redis adapters are obsolete and fail closed if set. Former ADR-0016 is not binding.
...
6. **OAuth broker.** Keep **in-memory** pending/codes/DCR only. MCP profile paths scale horizontally; `/oauth/*` may need a single replica or sticky routing until a later signed-state/CIMD change.
...
- Content-governance **form elicitation** reads client capabilities from the per-request `_meta` envelope (`io.modelcontextprotocol/clientCapabilities`); legacy `getClientCapabilities()` remains a fallback only when the same connection ran initialize (stdio via `serveStdio` / legacy HTTP shim). There is **no** process-local initialize-caps cache — sessionless HTTP clients must send form elicitation in `_meta` on each gated `tools/call` (including MRTR retries).
- OAuth authorize→callback→token remains process-local (document sticky `/oauth/*` or single instance).
- Breaking: Redis env vars fail closed; clients must re-send proposed on apply (already the case for write tool args).
