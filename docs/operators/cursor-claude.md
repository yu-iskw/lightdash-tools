...
Connect Cursor or Claude Code to a hosted `@lightdash-tools/mcp` server. Clients use **URL-only** config. Server OAuth setup and env reference: [mcp-oauth.md](mcp-oauth.md). Architecture: [ADR-0007](../adr/0007-mcp-http-transport-auth-modes-sdk-v2.md).

## Prerequisites

1. MCP HTTP server in hosted OAuth mode (see [mcp-oauth.md](mcp-oauth.md) — `LIGHTDASH_URL`, `LIGHTDASH_TOOLS_MCP_PUBLIC_URL`, `LIGHTDASH_TOOLS_OAUTH_CLIENT_ID`, `LIGHTDASH_TOOLS_OAUTH_CLIENT_SECRET`).
2. Lightdash OAuth app redirect URI registered as `{PUBLIC_URL}/oauth/callback`.
