...
**Excluded:** `POST …/query/metric-query`, `POST …/query/sql`, `POST …/query/underlying-data`, `POST …/query/{queryUuid}/download`, `POST …/query/{queryUuid}/schedule-download`, and all content/org/project mutations (create, update, delete, move, bulk-action).
